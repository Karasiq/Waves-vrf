...
public class VRFMiningModel {
    ...
    public static void main(String[] args) {
        ...
        try (FileWriter writer = new FileWriter("blocks-" + Block.MinTime + ".json", false)) {
            ...
            for (int i = 0; i < blocksCount; i++) {
                ...
                for (Block other : mined)
                    if (other.miner != block.miner) {
                        long delta = Math.abs(other.time - block.time);
                ...
                System.out.printf("Block generated: %d, bt = %d, change=%.2f%%\n", block.height, block.baseTarget, baseTargetChange);
            ...
            writer.append("]");
            double collisionsPct = ((double) collisions) / blocksCount * 100;
            double averageTime = (double) totalTime / blocksCount / 1000;
            if (removeMiner) averageTime *= 2;
