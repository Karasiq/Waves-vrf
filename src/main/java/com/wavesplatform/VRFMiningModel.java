...
public class VRFMiningModel {
    ...
    public static void main(String[] args) {
        ...
        final int collisionTime = Integer.parseInt(System.getProperty("collision-time", "1000"));
        ...
        long[] testnetBalances = new long[]{299471445709556L, 106074837858843L, 73927041108368L, 57616431578176L, 56674273054211L, 55241059031921L, 44074087570001L, 33339668292507L, 10073836540000L, 9709704600000L, 2805892840000L, 1595075131999L, 247281759745L, 246770580000L, 246172020039L, 244946660069L, 244608840036L, 244003460069L, 243964040036L, 243573220027L, 242461540066L, 241892000051L, 241337960027L};
        ...
        long[] stagenetBalances = {91497369370000L, 51295806540000L, 44987737640000L, 40930285420000L, 26772067600000L, 26178251670000L, 25175681420000L, 21344987850001L, 20898472160000L, 12822657260000L};

        for (int i = 0; i < balances.length; i++) {
            //balances[i] = (long) (balances[i] / 5);
        }
        //balances = stagenetBalances;
        ...
        double[] minersPerf = new double[balances.length], minersPredicted = new double[balances.length];
        for (int i = 0; i < miners.size(); i++) {
            long minerBalance = miners.get(i).balance;
            minersPredicted[i] = sumOfBlocks * minerBalance / (double) sumOfBalances;
            if (minersPredicted[i] > 0) minersPerf[i] = minersBlocks[i] / minersPredicted[i];
            ...
            System.out.printf("%.0fW (%.2f%%), %d blocks, %.2f%% perf\n", minerBalance * 1e-8, minerShare.multiply(BigDecimal.valueOf(100)), minersBlocks[i], minersPerf[i] * 100);
